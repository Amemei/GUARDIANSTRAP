...
(function() {
    ...
    Scene_Title.prototype.initializePseudo3DData = function() {
        ...
        for(var i = 0; i < this.commandLen_TSE; i++){
            this.data_Pseudo3D[i] = new NTMO.Data_Pseudo3D(param.centerX, param.centerY, speed,
                param.correction + (i - this.selected3DCommand) * (param.rotation),
                    param.offset, i, param.type);
        }
    };
    ...
    Scene_Title.prototype.nextChoice = function() {
        if(this.data_Pseudo3D[0].isRealAngleReachSpecifiedAngle()){
            this.selected3DCommand = (this.selected3DCommand + 1) % this.commandLen_TSE;
    ...
    Scene_Title.prototype.prevChoice = function() {
        if(this.data_Pseudo3D[0].isRealAngleReachSpecifiedAngle()){
            this.selected3DCommand = (this.selected3DCommand + (this.commandLen_TSE - 1)) % this.commandLen_TSE;
            SoundManager.playCursor();
        }
    };
    ...
    //=============================================================================
    NTMO.Data_Pseudo3D = function(centerX,centerY,speed,realAngle,rotateSize,controlNumber,type){
    ...
    NTMO.Data_Pseudo3D.prototype.initialize = function(centerX,centerY,speed,realAngle,rotateSize,controlNumber,type){
        ...
        if (this._type === NTMO.Circle_Type.TrueCircle)
        {
            ...
            this._real_scale = this._default_scale + Math.sin(this._radian) * this._range;
        }
        else if(this._type === NTMO.Circle_Type.Ellipse1)
        {
            this._radian     = this._real_angle    * Math.PI / 180.0;
            this._x          = this._center_x      + Math.cos(this._radian) * this._rotateSize / 2;
            this._y          = this._center_y      + Math.sin(this._radian) * this._rotateSize;
            this._real_scale = this._default_scale + Math.sin(this._radian) * this._range;
        }
        else
        {
            this._radian     = this._real_angle    * Math.PI / 180.0;
            this._x          = this._center_x      + Math.cos(this._radian) * this._rotateSize;
            this._y          = this._center_y      + Math.sin(this._radian) * this._rotateSize / 2;
            this._real_scale = this._default_scale + Math.sin(this._radian) * this._range;
        }        
    };
